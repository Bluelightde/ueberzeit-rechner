...
def create_icon(size=1024):
    ...
    bg_color      = (41, 98, 178)     # kräftiges Blau
    dark_blue    = (26, 60, 115)     # dunkleres Blau für Tiefe
    accent_color  = (255, 193, 7)     # Goldgelb für Überstunden-Zeiger

    r_bg = int(s * 0.18)
    rounded_rect(draw, [pad, pad, s - pad, s - pad], r_bg, bg_color)
    ...
    circle(draw, cx, cy, r_outer, fill=dark_blue)
    ...
    for h in range(12):
        ...
        line_at_angle(draw, cx, cy, angle, r_t_in, r_t_out, dark_blue, w)

    # --- Stunden-Zeiger (zeigt auf ~10 Uhr) ---
    r_hour = int(r_face * 0.54)
    line_at_angle(draw, cx, cy, 300, 0, r_hour, dark_blue, 26)

    # --- Minuten-Zeiger (zeigt auf ~12 Uhr) ---
    r_min = int(r_face * 0.76)
    line_at_angle(draw, cx, cy, 0, 0, r_min, dark_blue, 18)

    # --- Überstunden-Zeiger (goldgelb, zeigt auf ~2 Uhr = Mehrarbeit) ---
    r_over = int(r_face * 0.70)
    line_at_angle(draw, cx, cy, 60, 0, r_over, accent_color, 18)

    # --- Plus-Symbol oben rechts (Überstunden-Symbol) ---
    px = int(s * 0.72)
    py = int(s * 0.28)
    pr = int(s * 0.115)
    circle(draw, px, py, pr, fill=accent_color)

    arm = int(pr * 0.52)
    thick = int(pr * 0.28)
    draw.rectangle([px - thick, py - arm, px + thick, py + arm], fill=dark_blue)
    draw.rectangle([px - arm, py - thick, px + arm, py + thick], fill=dark_blue)

    # --- Mittelpunkt ---
    circle(draw, cx, cy, int(s * 0.025), fill=dark_blue)
...
def save_ico(img, path):
    """Speichert das Bild als ICO-Datei."""
...
def save_icns(img, path):
    """Speichert das Bild als ICNS-Datei."""
    chunks = []

...
# Persistenter Datenpfad: überlebt Rebuilds, Script und Binary teilen dieselbe DB.
if sys.platform == 'darwin':
    _data_home = os.path.expanduser('~/Library/Application Support')
elif sys.platform == 'win32':
    _data_home = os.environ.get('APPDATA', os.path.expanduser('~'))
else:
    _data_home = os.environ.get('XDG_DATA_HOME', os.path.join(os.path.expanduser('~'), '.local', 'share'))
BASE_DIR = os.path.join(_data_home, 'ueberstundenrechner')
...
from PyQt6.QtGui import QColor, QFont, QIcon, QPalette, QPainter, QPen
...
class DBManager:
    ...
    def insert(self, entry: WorkEntry):
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO entries (date, start, end, pause, minutes, reason)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (entry.date, entry.start, entry.end, entry.pause, entry.minutes, entry.reason))
        self.conn.commit()
    ...
    def update(self, entry: WorkEntry):
        cursor = self.conn.cursor()
        cursor.execute("""
            UPDATE entries SET date=?, start=?, end=?, pause=?, minutes=?, reason=? WHERE id=?
        """, (entry.date, entry.start, entry.end, entry.pause, entry.minutes, entry.reason, entry.id))
        self.conn.commit()
...
# --- ItemDelegate für Kalender-Heatmap (nur Heute-Rahmen) ---
...
class EditDialog(QDialog):
    ...
    def recalc_minutes(self):
        ...
        results, _ = calculate_timed_entries(other_timed + [current_temp],
                                             self.target_minutes, self.max_minutes, self.auto_break)
...
class UeberstundenApp(QMainWindow):
    ...
    def apply_theme(self):
        ...
        try:
            import qdarktheme
            css = qdarktheme.load_stylesheet(theme)
            # Items transparent → setBackground() der Kalender-Zellen wirkt wieder
    ...
    def auto_calculate_goal_hours(self):
        ...
        holidays = {}
        for y in years:
            holidays.update(get_holidays(y, state))

        while curr <= end_d:
            date_str = curr.toString("yyyy-MM-dd")
            if curr.dayOfWeek() <= 5 and date_str not in holidays:
                workdays += 1
            curr = curr.addDays(1)

        target_mins = self.get_target_minutes()
        total_hours_needed = (workdays * target_mins) / 60.0
    ...
    def update_goal_status(self):
        ...
        years = set([curr.year(), target_start_date.year()])
        holidays = {}
        for y in years:
            holidays.update(get_holidays(y, state))
    ...
    def update_calendar_heatmap(self):
        ...
        c_hl      = pal.color(QPalette.ColorRole.Highlight)
        c_hl_text = pal.color(QPalette.ColorRole.HighlightedText)
        c_weekend = pal.color(QPalette.ColorRole.AlternateBase)
            ...
            for col, day in enumerate(week):
                if day == 0:
                    ...
                    if is_holiday:
                        ...
                        if mins == 0:
                            item.setBackground(c_hl)
                            item.setForeground(c_hl_text)
                        elif mins > 0:
                    ...
                    if is_today:
                        ...
                        item.setText(f"{day}\nHeute{hol_suffix}" + (f"\n({'+' if mins > 0 else ''}{mins}m)" if mins != 0 else ""))
                        item.setBackground(c_hl)
                        item.setForeground(c_hl_text)
                    else:
                        item.setData(Qt.ItemDataRole.UserRole + 1, False)
    ...
    def _frage_ja_nein(self, titel: str, text: str) -> bool:
    ...
    def delete_entry(self, entry: WorkEntry):
        ...
        if self._frage_ja_nein("Löschen bestätigen", f"Eintrag vom {d} wirklich löschen?"):
    ...
    def import_csv(self):
        ...
        try:
            ...
            if not self._frage_ja_nein("Import bestätigen",
